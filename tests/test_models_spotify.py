...
def test_create_spotify_track():
    ...
    assert spotify_track.explicit == False
    assert spotify_track.artists == [SpotifyArtist.from_json(artist_data) for artist_data in spotify_track_sample.get("artists", [])]
    assert spotify_track.album == SpotifyAlbum.from_json(spotify_track_sample.get("album"))
    assert spotify_track.duration_ms == 1000
    assert spotify_track.is_playable == True
    assert spotify_track.is_local == False
...
def test_create_spotify_album():
    ...
    assert spotify_album.artists == [SpotifyArtist.from_json(artist_data) for artist_data in spotify_album_sample.get("artists", [])]
...
def test_create_spotify_playlist():
    ...
    assert spotify_playlist.public == True
    assert spotify_playlist.collaborative == False
    assert spotify_playlist.tracks == [SpotifyTrack.from_json(track_data.get("track")) for track_data in spotify_playlist_tracks_sample if track_data.get("track")] if spotify_playlist_tracks_sample else []
